...
def app_instance():
    ...
    return GithubApp(integration=mock_integration)

def test_universal_mcp_github_app_initialization(app_instance):
    """
    Test that the GithubApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "github", "GithubApp instance has unexpected name."


def test_universal_mcp_github_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GithubApp has a well-formatted docstring,
...
def test_universal_mcp_github_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GithubApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
